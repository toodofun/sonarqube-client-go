...
func newVersion(s string) *version {
	...
	if err := v.UnmarshalJSON([]byte(s)); err != nil {
		_ = v.UnmarshalJSON([]byte(defaultVersionString))
	}
	return v
}
...
func webservicesListURL(host string, internal bool) string {
...
func getTargetVersion(client *http.Client, host, version, auth string) (string, error) {
	if version != "" {
		return version, nil
	}
	req, err := http.NewRequest(http.MethodGet, host+serverVersionUrl, nil)
	if err != nil {
	...
	if resp.StatusCode == http.StatusUnauthorized {
		return "", errors.New("authorization failed to fetch server version")
	}
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("got error response from the server, code - %d", resp.StatusCode)
	}
...
func getDefinition(
	...
	req, _ := http.NewRequest("GET", webservicesListURL(host, internal), nil)
	if auth != "" {
		req.Header.Set("Authorization", auth)
	}
	resp, err := client.Do(req)
...
func loadAPI(
	...
	version, err := getTargetVersion(client, host, version, auth)
	if err != nil {
	...
	enrichResponseSchemas(client, host, auth, def)
